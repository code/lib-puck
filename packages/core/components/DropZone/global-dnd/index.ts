...
`
  ...
  const DIRECTION_SMOOTHING_WINDOW = 120;
    ...
    requestAnimationFrame(() => {
      ...
      if (state === "IDLE") {
        ...
        for (let i = 0; i < sortedByX.length; i++) {
          ...
          // TODO consider switching edge / center logic for more natural impacts
          let candidateEdge = {
          ...
          if (axis === "x") {
            if (direction === "down") {
              candidateEdge.x = candidate.rect.right; // moving left
            } else if (direction === "up") {
              candidateEdge.x = candidate.rect.left; // moving right
            }
          } else {
            if (direction === "down") {
              candidateEdge.y = candidate.rect.bottom; // moving up
            } else if (direction === "up") {
              candidateEdge.y = candidate.rect.top; // moving down
            }
          }

          // TODO calculate ahead of time
          const dragCenter = {
            x: transformedRect.left + drag.rect.width / 2,
            y: transformedRect.top + drag.rect.height / 2,
          };

          const distance = calculateDistance(dragCenter, candidateEdge);

          const existingDistance = impact
            ? calculateDistance(dragCenter, impact.edge)
            : 0;
          ...
          if (axis === "x") {
            position =
              dragCenter.x > candidateEdge.x ? "afterend" : "beforebegin";
          } else {
            position =
              dragCenter.y > candidateEdge.y ? "afterend" : "beforebegin";
          }
